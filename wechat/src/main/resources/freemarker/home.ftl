<#if $version> <#assign $version='prod'/> </#if>
<!DOCTYPE HTML>
<html>
<head>
...
<body ng-app="layoutApp">
	<div class="content" ng-controller="contentController">
		...
		<link rel="stylesheet" href="/static/css/business/home.css?random=$version"></link>
			...
			<div ng-show="page=='PLAN'">
				<div ng-hide="joiningPlan">
					...
					<div ng-repeat="plan in plans" style="padding: 10px; border-bottom: 1px solid #dddddd;">
						<span>{{plan.name.message}} </span><a ng-hide="plan.joined" href="javascript:void(0)" ng-click="join(plan)"><span>#button("JOIN")</span></a><span
							ng-show="plan.joined"
						>#label("JOINED")</span>
					</div>
				</div>
		...
		<script src="/static/js/business/home.js?random=$version"></script>
	</div>
</body>
</html>